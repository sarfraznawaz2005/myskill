...
describe('Create Command', () => {
  ...
  it('should create a skill in non-interactive mode', async () => {
    ...
    expect(mockConsoleLog).toHaveBeenCalledWith(expect.stringContaining('Skill created successfully'));
  });
  ...
  it('should fail if required arguments are missing in non-interactive mode', async () => {
    ...
    expect(mockConsoleError).toHaveBeenCalledWith(expect.stringContaining('Error: --platform, --name, and --description are required'));
  ...
  it('should fail if name does not match platform schema', async () => {
    ...
    expect(mockConsoleError).toHaveBeenCalledWith(expect.stringContaining('Validation Error'));
    expect(process.exit).toHaveBeenCalledWith(1);
  });
});
